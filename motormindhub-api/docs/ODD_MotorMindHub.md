...
**Nota (andamentoPubblicazioni/andamentoCategorie/andamentoApprovazioni)** Stesso pattern di clamp/zero-fill/fuso orario di GestioneAmministrazioneUtenti.andamentoVisite/andamentoRegistrazioni (§2.5): il parametro `giorni` (query param HTTP, default 30) è vincolato lato service a [1, 90], ciascun metodo restituisce sempre esattamente `giorni` punti (uno per giorno di calendario, oggi incluso), e il bucket giornaliero è calcolato in fuso Europe/Rome. Endpoint REST: `GET /api/v1/autori/statistiche-autori/andamento-pubblicazioni?giorni=`, `GET /api/v1/autori/statistiche-autori/andamento-categorie?giorni=` e `GET /api/v1/autori/statistiche-autori/andamento-approvazioni?giorni=`, tutti `@PreAuthorize("hasRole('MANAGER_AUTORI')")` come il resto di `AutoriController`.
