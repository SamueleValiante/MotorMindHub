...
|-------------------------------|---------------------------------------------------------------------------------------------|
| **Metodo (query)**            | **Descrizione**                                                                             |
| searchArticles(criteria)      | Ricerca full-text (PostgreSQL tsvector/GIN, ambito Titolo+Tag — il corpo dell'articolo non vi rientra) combinata con filtri di categoria. L'ultimo termine della query usa in aggiunta il prefix-matching (`:*`), i precedenti restano a matching pieno/stemmed (cfr. nota sotto e ArticoloRepository.cercaPubblicati). (cfr. RF1.2) |
...
**Nota (searchArticles / ambito ricerca full-text e prefix-matching, V19)** `search_vector` (SDD 3.3) copre solo Titolo e Tag di Articolo: il corpo (Testo) ne è stato escluso con una migrazione dedicata (decisione di prodotto, ribalta il comportamento precedente). `ArticoloRepository.cercaPubblicati` applica inoltre il prefix-matching (`:*`) al solo ULTIMO termine della query testuale, mentre i termini precedenti restano a matching pieno/stemmed via `websearch_to_tsquery` come prima — necessario perché la ricerca di Esplora è live (debounced, non gated dal submit): senza prefix-matching, una parola troncata a metà digitazione (es. "past" mentre si scrive "pastiglie") non stemma sulla forma completa e mostrerebbe 0 risultati per gran parte della battitura. Costruzione sicura contro l'injection di sintassi tsquery (`&`, `|`, `!`, `(`, `)`): il testo utente non raggiunge mai direttamente il parser rigido di `to_tsquery` — l'ultimo termine è prima tokenizzato con `to_tsvector` (che scarta i caratteri speciali come rumore, non li interpreta come operatori), e solo il/i lessema/i già ripuliti che ne escono vengono concatenati con `:*` e ripassati a `to_tsquery`. Se l'ultimo termine non produce alcun lessema (stopword italiana, sola punteggiatura), un `COALESCE` lo riduce a un tsquery vuoto, neutro rispetto all'AND (`&&`) con i termini precedenti, così la ricerca non si azzera.
